...
function saveAndOpenErrorLog() {
  try {
    ...
    const logFileName = `nox-api-crash-${timestamp}.log`;
...
function analyzeError(errorLogs) {
  ...
  if (allLogs.includes('database is locked') || 
    ...
    return {
      ...
      solution: '可能的解决方案：\n\n1. 检查是否已经打开了另一个 Nox API 窗口\n   - 查看任务栏/Dock 中是否有其他 Nox API 图标\n   - 查看系统托盘（Windows）或菜单栏（Mac）中是否有 Nox API 图标\n\n2. 如果刚刚关闭过应用，请等待 10 秒后再试\n\n3. 重启电脑以释放被占用的文件\n\n4. 如果问题持续，可以尝试：\n   - 退出所有 Nox API 实例\n   - 删除数据目录中的临时文件（.db-shm 和 .db-wal）\n   - 重新启动应用'
    };
  }
...
function getBinaryPath() {
  ...
  if (isDev) {
    const binaryName = platform === 'win32' ? 'nox-api.exe' : 'nox-api';
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    env.SQLITE_PATH = path.join(dataDir, 'nox-api.db');
