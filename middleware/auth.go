...
func authHelper(c *gin.Context, minRole int) {
	...
	// get header Nox-Api-User
	apiUserIdStr := c.Request.Header.Get("Nox-Api-User")
	if apiUserIdStr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "无权进行此操作，未提供 Nox-Api-User",
		})
		c.Abort()
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "无权进行此操作，Nox-Api-User 格式错误",
		})
		c.Abort()
		return
	...
	if id != apiUserId {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "无权进行此操作，Nox-Api-User 与登录用户不匹配",
		})
		c.Abort()
		return
	}
