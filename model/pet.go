...
func SeedPetData() {
	...
	items := []PetItem{
		...
		{
			Name:        "高级饲料",
			Description: "精心调配的高级饲料，恢复大量饥饿值并提升心情",
			Type:        "food",
			Rarity:      "R",
			Price:       150000,
			Enabled:     true,
		...
		{
			Name:        "能量果",
			Description: "富含能量的水果，同时恢复饥饿和心情",
			Type:        "food",
			Rarity:      "N",
			Price:       100000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 20, "mood": 20}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "清洁喷雾",
			Description: "温和的清洁喷雾，让宠物焕然一新",
			Type:        "potion",
			Rarity:      "N",
			Price:       75000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"cleanliness": 40}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "心情糖果",
			Description: "甜蜜的糖果，大幅提升宠物心情",
			Type:        "potion",
			Rarity:      "N",
			Price:       100000,
			Enabled:     true,
		...
		{
			Name:        "全效药水",
			Description: "珍贵的全效药水，同时恢复饥饿、心情和清洁度",
			Type:        "potion",
			Rarity:      "SR",
			Price:       250000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 30, "mood": 30, "cleanliness": 30}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
	}
