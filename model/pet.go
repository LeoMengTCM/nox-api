...
func petSeedItems(now int64) []PetItem {
	return []PetItem{
		...
		// ========== Food — Extended (HP sweets & treats) ==========
		{
			Name:        "冰鼠",
			Description: "蜂蜜公爵出品的冰冻小鼠糖，在肚子里嘎吱作响",
			Type:        "food",
			Rarity:      "N",
			Price:       40000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 25}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "嘶嘶蜂蜜糖",
			Description: "吃下去能让你飘浮起来的蜜蜂形巧克力，嗡嗡作响",
			Type:        "food",
			Rarity:      "N",
			Price:       35000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 20, "mood": 5}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "咬人馅饼",
			Description: "坩埚形状的蛋糕，小心——它可能会咬你一口！",
			Type:        "food",
			Rarity:      "R",
			Price:       80000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 35}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "酸味棒棒糖",
			Description: "酸到能在舌头上烧出洞的极限糖果，不建议长时间舔",
			Type:        "food",
			Rarity:      "N",
			Price:       25000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 15, "mood": 10}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "巧克力釜饼",
			Description: "坩埚造型的巧克力外壳，里面装满了火焰威士忌奶油",
			Type:        "food",
			Rarity:      "R",
			Price:       90000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 40, "mood": 15}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "甘草魔杖",
			Description: "魔杖形状的甘草糖，嚼着吃还能发出小火花",
			Type:        "food",
			Rarity:      "N",
			Price:       45000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 25, "mood": 5}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "吹宝超级泡泡糖",
			Description: "能吹出永不破裂的蓝色泡泡，充满房间长达数日",
			Type:        "food",
			Rarity:      "N",
			Price:       20000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 15}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "岩皮蛋糕",
			Description: "海格亲手烤制的特色点心，硬如石头但满含心意",
			Type:        "food",
			Rarity:      "N",
			Price:       15000,
			Enabled:     true,
		...
		{
			Name:        "南瓜馅饼",
			Description: "霍格沃茨特快列车上的热门零食，金黄酥脆",
			Type:        "food",
			Rarity:      "R",
			Price:       70000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 35, "mood": 10}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "蜂蜜公爵巧克力",
			Description: "蜂蜜公爵的镇店之宝，丝滑醇厚的顶级巧克力",
			Type:        "food",
			Rarity:      "SR",
			Price:       150000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 50, "mood": 20}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		// ========== Potions — Extended ==========
		{
			Name:        "胡椒提神剂",
			Description: "庞弗雷夫人的招牌药剂，喝完耳朵冒蒸汽精神大振",
			Type:        "potion",
			Rarity:      "R",
			Price:       130000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 50}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "安神汤剂",
			Description: "能让最焦躁的宠物也平静下来的温和药剂",
			Type:        "potion",
			Rarity:      "R",
			Price:       110000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 40, "cleanliness": 10}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "治愈药膏",
			Description: "白鲜精华，涂抹后伤口立即愈合，全面恢复状态",
			Type:        "potion",
			Rarity:      "SR",
			Price:       180000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"hunger": 30, "mood": 30}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "魔力增强药",
			Description: "增强智慧的魔药，让宠物头脑清醒活力充沛",
			Type:        "potion",
			Rarity:      "SR",
			Price:       250000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 60}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "万能解药",
			Description: "对付常见毒素的基础解药，还能清除身上的脏污",
			Type:        "potion",
			Rarity:      "N",
			Price:       60000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"cleanliness": 35}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		// ========== Toys (type="potion", mood-focused) ==========
		{
			Name:        "飞天扫帚模型",
			Description: "光轮2000的精致缩小版，宠物最爱追着它跑",
			Type:        "potion",
			Rarity:      "R",
			Price:       150000,
			Enabled:     true,
		...
		{
			Name:        "魁地奇球套装",
			Description: "迷你游走飞球+金色飞贼，宠物的终极玩具",
			Type:        "potion",
			Rarity:      "SR",
			Price:       300000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 70}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "巫师棋",
			Description: "会自己移动的棋子，和宠物对弈永远不会无聊",
			Type:        "potion",
			Rarity:      "R",
			Price:       200000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 55}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "记忆球",
			Description: "忘了什么事的时候会变红——虽然你还是想不起来忘了啥",
			Type:        "potion",
			Rarity:      "R",
			Price:       120000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 35}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "熄灯器",
			Description: "邓布利多的遗物，能收集和释放光球，宠物的夜灯",
			Type:        "potion",
			Rarity:      "SSR",
			Price:       400000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 80}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "速记羽毛笔",
			Description: "丽塔·斯基特同款自动书写笔，宠物喜欢追着它跑",
			Type:        "potion",
			Rarity:      "N",
			Price:       50000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 20}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "韦斯莱烟花",
			Description: "韦斯莱魔法把戏坊出品，璀璨烟花让宠物兴奋不已（有点呛）",
			Type:        "potion",
			Rarity:      "SR",
			Price:       250000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 65, "cleanliness": -10}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			Name:        "妖怪书",
			Description: "《妖怪们的妖怪书》——记得先抚摸书脊再打开！",
			Type:        "potion",
			Rarity:      "R",
			Price:       160000,
			Enabled:     true,
			Effect:      mustMarshal(map[string]int{"mood": 40}),
			CreatedAt:   now,
			UpdatedAt:   now,
		},
	}
