...
/* ─── Looping Typewriter ─── */
function LoopingTypewriter({ text, baseDelay = 70, holdDelay = 4000, eraseDelay = 35, pauseDelay = 1200 }) {
  ...
  useEffect(() => {
    ...
    if (phase === 'typing') {
      if (display.length < text.length) {
        const char = text[display.length];
        let delay = baseDelay;
        if (char === '.' || char === ',') delay *= 4;
        else if (char === ' ') delay *= 0.5;
        else delay += Math.random() * 30;
        timer = setTimeout(() => setDisplay(text.slice(0, display.length + 1)), delay);
      } else {
        timer = setTimeout(() => setPhase('erasing'), holdDelay);
      }
    } else {
      if (display.length > 0) {
        timer = setTimeout(() => setDisplay(display.slice(0, -1)), eraseDelay);
      } else {
        timer = setTimeout(() => setPhase('typing'), pauseDelay);
      }
    }

    return () => clearTimeout(timer);
  }, [display, phase, text, baseDelay, holdDelay, eraseDelay, pauseDelay]);
  ...
  useEffect(() => {
    const tryLoad = async () => {
      for (const ext of ['jpg', 'png', 'webp', 'jpeg', 'gif']) {
        try {
          const r = await fetch(`/api/user/avatar/1.${ext}`, {
            method: 'HEAD',
          });
          if (r.ok) {
            setAvatarSrc(`/api/user/avatar/1.${ext}`);
            return;
          }
        } catch {
          // try next
      ...
      <section className="relative pt-24 pb-16 md:pt-36 md:pb-24">
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-6 text-center">
          ...
          <motion.p
            ...
            className="text-lg md:text-xl text-text-secondary mt-6 leading-relaxed"
          >
            什么都懂一点，生活更精彩一点。
          </motion.p>
          ...
          <motion.p
            ...
            className="text-sm text-text-tertiary mt-3 leading-relaxed max-w-md mx-auto"
          >
            中医博士，写点代码。
            <br />
            这是我为家人朋友们搭建的 AI 服务站。
          ...
          >
            <Link
              to="/console"
              className="inline-flex items-center gap-2 rounded-full bg-accent px-7 py-2.5 text-sm font-medium text-text-inverse shadow-sm hover:bg-accent-hover active:bg-accent-active transition-colors no-underline"
            >
              开始使用
      ...
      <section ref={quoteRef} className="relative py-20 md:py-28 overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto px-6 text-center">
          <div className="py-10 md:py-14 border-y border-accent/15">
            <p className="italic text-xl md:text-3xl lg:text-4xl text-text-primary leading-relaxed tracking-tight" style={{ fontFamily: "'EB Garamond', 'Noto Serif SC', Georgia, serif" }}>
              {quoteInView ? (
                <LoopingTypewriter
